...
export const BoardCanvas: React.FC<CanvasProps> = ({ currentUser, onSignOut, onUserUpdated }) => {
  ...
  const [isP2PConnected, setIsP2PConnected] = useState<boolean>(p2pSync.isConnectedToPartner);
  const [activeRoomCode, setActiveRoomCode] = useState<string>(p2pSync.roomCode);

  useEffect(() => {
    const unsub = p2pSync.onConnectionStateChange((connected) => {
      setIsP2PConnected(connected);
      setActiveRoomCode(p2pSync.roomCode);
    });
    return unsub;
  }, []);
  ...
  useEffect(() => {
    ...
    const unsubRequestState = realtimeClient.on('p2p:request_state', () => {
      p2pSync.send({
        type: 'state:snapshot',
        snapshot: {
    ...
    const unsubApplySnapshot = realtimeClient.on('p2p:apply_snapshot', (snapshot: any) => {
      if (snapshot && snapshot.boardId === activeBoardId) {
  ...
  return (
    ...
    >
      ...
      <header className="relative z-30 bg-[#160d0a] border-b-2 border-[#4a2e20] px-3 sm:px-4 py-2 flex items-center justify-between shadow-lg gap-3 flex-wrap sm:flex-nowrap">
        ...
        <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap">
          ...
          <button
            ...
            title="Click to view Sheriff Dispatch Radio Frequency & Partner Link"
          >
            {isP2PConnected ? (
              <>
                <span className="w-2 h-2 rounded-full bg-emerald-400 animate-ping" />
                <Radio className="w-3 h-3 text-emerald-400" />
                <span className="hidden lg:inline font-bold">Partner Linked (Direct P2P)</span>
              </>
            ) : (
              <>
                <span className="w-2 h-2 rounded-full bg-amber-400" />
                <Radio className="w-3 h-3 text-amber-400" />
                <span className="hidden lg:inline">Dispatch: {activeRoomCode}</span>
              </>
            )}
      ...
      {/* Direct P2P & Realtime Connection Modal */}
      {isConnModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fadeIn">
          <div className="w-full max-w-lg bg-[#1a0f0b] border-2 border-[#5a3928] rounded-2xl p-6 shadow-2xl text-[#f5ebd4] space-y-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between pb-3 border-b border-[#382216]">
              <div>
                <h3 className="font-display font-bold text-lg text-[#f5ebd4] tracking-wide flex items-center gap-2">
                  <Radio className="w-5 h-5 text-[#e5c158]" />
                  <span>Sheriff Dispatch & Live P2P Link</span>
                </h3>
                <p className="font-editorial italic text-xs text-[#a08774] mt-0.5">
                  Direct WebRTC synchronization connecting Michigan and Alabama in real time.
                </p>
              </div>
              <button
                type="button"
            ...
            {/* Direct P2P Status Box */}
            <div
              className={`p-4 rounded-xl border font-typewriter text-xs space-y-2 ${
                isP2PConnected
                  ? 'bg-emerald-950/60 border-emerald-500 text-emerald-200'
                  : 'bg-[#25150e] border-[#5a3928] text-amber-200'
              }`}
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 font-bold text-sm">
                  <span className={`w-3 h-3 rounded-full ${isP2PConnected ? 'bg-emerald-400 animate-ping' : 'bg-amber-400'}`} />
                  <span>{isP2PConnected ? 'DIRECT P2P LINK ACTIVE' : 'LISTENING ON RADIO CHANNEL'}</span>
                </div>
                <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-black/40 border border-current">
                  {isP2PConnected ? 'Connected (0ms Relay)' : 'Waiting for Partner'}
                </span>
              </div>
              <p className="text-[11px] opacity-90 leading-relaxed text-[#cfb69b]">
                {isP2PConnected
                  ? 'Your computer and your partner are connected directly via WebRTC DataChannel. Card drags, red yarn strings, and video calls pass straight between both browsers!'
                  : 'Your radio channel is active. Share the partner link with your co-investigator so they can open the board from Michigan, Alabama, or anywhere else.'}
              </p>
            </div>

            {/* Dispatch Room Code Setting */}
            <div className="bg-[#120906] border border-[#44281a] rounded-xl p-4 space-y-3 font-typewriter text-xs">
              <div className="flex items-center justify-between">
                <span className="font-bold text-[#e5c158] uppercase flex items-center gap-1.5">
                  <Radio className="w-3.5 h-3.5" />
                  Dispatch Frequency / Room
                </span>
                <span className="text-[11px] font-mono text-[#a08774]">{activeRoomCode}</span>
              </div>
              <div className="flex items-center gap-2">
                <input
                  type="text"
                  ...
                  onChange={(e) => {
                    ...
                    p2pSync.setRoomCode(clean);
                  }}
                  placeholder="twinpeaks-sheriff-case"
                  className="w-full bg-[#0a0503] border border-[#5a3928] rounded-lg px-3 py-2 text-xs font-mono text-[#f5ebd4] focus:outline-none focus:border-[#e5c158]"
                />
                <button
                  type="button"
                  onClick={() => setIsShareModalOpen(true)}
                  className="bg-[#83161c] hover:bg-[#991b1b] text-white px-3.5 py-2 rounded-lg font-bold text-xs uppercase tracking-wider flex items-center gap-1.5 whitespace-nowrap cursor-pointer border border-[#b91c1c]"
                >
                  <Share2 className="w-3.5 h-3.5" />
                  <span>Share Link</span>
                </button>
              </div>
              <p className="text-[11px] text-[#8f745f]">
                Both partners must be on the same Dispatch Frequency to see each other.
              </p>
            </div>
            ...
            >
              <div className="text-[11px] font-typewriter text-[#a08774] font-bold uppercase tracking-wider">
              ...
              <div className="flex items-center justify-between pt-3 border-t border-[#382216]">
                ...
                <button
                  ...
                  className="bg-[#2d170f] hover:bg-[#3d1f15] text-[#fdf2f2] font-typewriter text-xs uppercase tracking-wider px-4 py-2 rounded-lg border border-[#5a3928] cursor-pointer flex items-center gap-2 font-bold transition-all disabled:opacity-50"
                >
                  {isTestingConn ? 'Testing Connection...' : 'Save Database Keys'}
                </button>
              </div>
            </form>
          </div>
